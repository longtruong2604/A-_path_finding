...
import os
import pygame as pg
from src.ui import ImageSprite, TextSprite
...
class Intro(State):
    ...
    def boot(self):
        ...
        self.game.audio.load_sound(
            "intro", os.path.join("assets", "sounds", "intro.wav")
        )
        # add a background image
        self.all_sprites.add(
            ImageSprite(self.game, os.path.join("assets", "images", "test.jpg"))
        )
        # add some text
        text = TextSprite(
            self.game,
        ...
        # add some more text
        any_key_text = TextSprite(
            self.game,
            os.path.join("assets", "fonts", "PressStart2P-Regular.ttf"),
            "Press any key to switch to Outro",
            12,
            color=(255, 255, 255),
        )
